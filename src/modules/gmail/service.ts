...
export class GmailService {
  ...
  async createDraft({ 
    ...
    try {
      ...
      // Get full message details
      const messageResponse = await this.getEmails({
        email,
    ...
    try {
      ...
      const drafts = await Promise.all(
        data.drafts.map(async (draft) => {
          const { data: draftData } = await gmail.users.drafts.get({
            userId: 'me',
            id: draft.id!,
            format: 'full',
          });
      ...
      const response: GetGmailSettingsResponse = {
        ...
        settings: {
          ...
          imap: {
            ...
            autoExpunge: imap.autoExpunge != null ? imap.autoExpunge : undefined,
            expungeBehavior: imap.expungeBehavior ?? undefined,
            maxFolderSize: imap.maxFolderSize != null ? imap.maxFolderSize : undefined
          },
          language: {
